...
$(document).ready(function () {
    ...
    $('#addTime').on('click', function () {
        ...
        // Adjusted local time variables for am and pm formats
        const dateNN = new Date();
        window.milliAdd = interval * 60 * 60 * 1000;
        dateNN.setMilliseconds(dateNN.getMilliseconds() + window.milliAdd);
        const hourNN = dateNN.getHours();
        const minuteNN = dateNN.getMinutes();

        // Adjusted Time conditional to render text
        if (hourNN >= 12) {
            // Creating a DIV and Paragraph element and saving it to a variable
            const currentD = document.createElement('DIV');
            const currentP = document.createElement('P');
            ...
            // Creating a text node using string interpolation to show the current time and PM
            const local = document.querySelector('.local');
            const pmText = document.createTextNode(`The current time is ${hourNN}:${minuteNN} P.M`);
            ...
            const local = document.querySelector('.local');
            const amText = document.createTextNode(`The current time is ${hourNN}:${minuteNN} A.M`);
        ...
        // Variables for adjusted am pm 
        var mN = moment().tz("America/Los_Angeles");
        if (window.milliAdd2) {
            mN = moment().add(window.milliAdd2, "ms").tz("America/Los_Angeles");
        }
        var nHrC = mN.hour();
        var nMinC = mN.tz("America/Los_Angeles").minute();
        ...
        if (nHrC >= 12) {
            ...
            // Creating text node to show time.
            const localC = document.querySelector('.california');
            const pmText = document.createTextNode(`The current time is ${nHrC}:${nMinC} P.M`);
            // appending to render time
            currentDI.appendChild(currentPA);
            localC.appendChild(currentDI);
            currentPA.appendChild(pmText);
            ...
            const localC = document.querySelector('.california');
            const amText = document.createTextNode(`The current time is ${nHrC}:${nMinC} A.M`);
            currentDI.appendChild(currentPA);
            localC.appendChild(currentDI);
            currentPA.appendChild(amText);
        } else {
        ...
        // Variables for adjusted am pm formats
        var mM = moment().tz("Europe/London");
        if (window.milliAdd3) {
            mM = moment().add(window.milliAdd3, "ms").tz("Europe/London");
        }
        var loHrNN = mM.hour();
        var loMinNN = mM.tz("Europe/London").minute();

        // London Adjusted time Conditional to render am or pm formats
        if (loHrNN >= 12) {
            // Creating P element
            const currentLD = document.createElement('DIV');
            const currentLP = document.createElement('P');
            ...
            // Creating text node to show time.
            const localLo = document.querySelector('.london');
            const pmText = document.createTextNode(`The current time is ${loHrNN}:${loMinNN} P.M`);
            // appending to render time
            currentLD.appendChild(currentLP);
            localLo.appendChild(currentLD);
            currentLP.appendChild(pmText)
            ...
            const localLo = document.querySelector('.london');
            const amText = document.createTextNode(`The current time is ${loHrNN}:${loMinNN} A.M`);
